...
def agent(obs, config):
    ...
    for ship in me.ships:
        ...
        if ship.next_action == None:
            ...
            if exploration % 50 == 0 and len(me.shipyards) < 5:
            ...
            if ship_state.current_state == ship_state.collect:
                ...
                if ship.cell.halite < 100:
                        ...
                        #Trys four times to move to a new position
                        for i in range(4):
            ...
            if ship_state.current_state == ship_state.deposit:
                ...
                nearest_shipyard = min(me.shipyards, key=lambda x: sum(abs(ship.position - x.position)))
                direction = getDirTo(ship.position, nearest_shipyard.position, size)
                ...
                if ship.position == me.shipyards[0].position and ship.halite == 0:
    ...
    for shipyard in me.shipyards:
        occupied = False
        for ship in me.ships:
            if ship.position == shipyard.position and shipyard.position not in ship_pos:
                occupied = True
        if not occupied and len(me.ships) < 10:
